...
fn zero_filter(old_size: usize, new_size: usize, h: &mut [Vec<FftData>]) {
    // PATCH(joi): the adaptive filter shrinks when the echo path shortens (e.g. after a barge-in
    // flush or an audio-thread stall), so `set_size_partitions` calls this with `old_size >
    // new_size`. Upstream then slices `h[old..new]` with start > end and panics, killing the
    // capture thread. There are no *new* partitions to clear when shrinking, so this is a no-op.
